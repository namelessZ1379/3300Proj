 ...
 * chThdSleepMilliseconds(unsigned long num_ms)
...
/*
#define chThdSleepMilliseconds    chThdSleepMilliseconds
...
//}
#define chprintf(CHP, 	printf	//��ӡ��Ϣ
...
#define chThdSleepMilliseconds    msp430_chThdSleepMilliseconds
...
int mpu_init(void)
{
    ...
    if (rev) {
        ...
        if (rev == 1)
            ...
            chprintf(CHP,"Unsupported software product rev %d.\n", rev);
            return -1;
        }
    } else {
        ...
        rev = data[0] & 0x0F;
        if (!rev) {
            chprintf(CHP,"Product ID read as 0 indicates device is either "
                "incompatible or an MPU3050.\n");
            return -1;
        } else if (rev == 4) {
            chprintf(CHP,"Half sensitivity part found.\n");
            st.chip_cfg.accel_half = 1;
        } else
            st.chip_cfg.accel_half = 0;
    }
...
static int get_st_biases(long *gyro, long *accel, unsigned char hw_test)
{
    ...
    if (i2c_write(st.hw->addr, st.reg->fifo_en, 1, data))
        return -1;
    chThdSleepMilliseconds(test.wait_ms);
    data[0] = 0;
    if (i2c_write(st.hw->addr, st.reg->fifo_en, 1, data))
        return -1;
...
int setup_compass(void)
{
    ...
    if (akm_addr > 0x0F) {
        ...
        chprintf(CHP,"Compass not found.\n");
        return -1;
    }
...
//����ֵ:0,����
//    ����,ʧ��
uint8_t mpu_dmp_get_data(float* P_R_Y)
{
  ...
  //palTogglePad(GPIOD,GPIOD_LED3);
    ...
    q0 = quat[0] / q30;	//q30��ʽת��Ϊ������
		...
		q3 = quat[3] / q30;
		//�����õ�������/������/������
		P_R_Y[0] = asin(-2 * q1 * q3 + 2 * q0* q2)* 57.3;	// pitch
		P_R_Y[1] = atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2* q2 + 1)* 57.3;	// roll
		P_R_Y[2]   = atan2(2*(q1*q2 + q0*q3),q0*q0+q1*q1-q2*q2-q3*q3) * 57.3;	//yaw
	}else return 2;
	return 0;
}
